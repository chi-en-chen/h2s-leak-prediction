...
油井 H2S 泄漏扩散 - 核心可视化元素 (SLAB 重气模型输出)
 ...
 1) 传感器点位: 4 个传感器位于油井正北/正西/正南/正东, 距井口可调
...
# 传感器布局: 正北/正西/正南/正东, 距井口可调(由 runner.py 动态更新) (最新要求)
...
def draw_panel(ax, field, params, t, beta_deg):
    ...
    sd = max(params.get("sensor_dist", 60.0), 60.0)
    ...
    for r in sensor_readings(field, params, t):
        ...
        # Halo circle behind sensor for visibility

        circle = plt.Circle((r["E"], r["N"]), sd * 0.045, facecolor=fc,

                            edgecolor="white", linewidth=2.0, alpha=0.35, zorder=6)

        ax.add_patch(circle)

        ax.plot(r["E"], r["N"], "D", ms=30, color=fc, mec="white", mew=2.2,

                zorder=7)

        txt = f"{r['name']} {r['desc']} {r['ppm']:.2f} ppm"

        ax.text(r["E"], r["N"] + sd * 0.10, txt, ha="center", fontsize=11,

                color=fc, fontweight="bold", zorder=8,

                bbox=dict(boxstyle="round,pad=0.3", fc="white", ec=fc,
    ...
    # ---- 自适应刻度: 以传感器距离为基准, 不同 sensor_dist 产生不同刻度 ----
    ...
    ideal_step = plot_range / target_n

    mult = max(1, round(ideal_step / base))

    major_step = mult * base

    n_ticks = plot_range / major_step

    if n_ticks > 18:

        major_step = base * int(n_ticks / 12 + 0.5)
    ...
    ax.grid(True, which="major", ls=":", color="#999999", alpha=0.6, lw=0.8, zorder=1)

    ax.grid(True, which="minor", ls=":", color="#999999", alpha=0.30, lw=0.5, zorder=1)

    ax.tick_params(axis="both", which="major", labelsize=8, length=4, width=0.8)

    ax.tick_params(axis="both", which="minor", labelsize=6, length=2, width=0.6)
    ...
    ax.set_title(
        ...
        f"t = {t/60:.0f} min   peak {np.max(C):.3g} ppm   1 ppm front {front:.0f} m",
...
def legend_handles():
    ...
    h = [
        ...
        Line2D([0], [0], marker="D", color="w", markerfacecolor="#2E9E44",
               ...
               markeredgecolor="white", ms=14, label="传感器 正常 (绿)"),

        Line2D([0], [0], marker="D", color="w", markerfacecolor="#D50000",

               markeredgecolor="white", ms=14, label="传感器 告警 (红, ≥10 ppm)"),
...
def write_sensor_csv(field, params, times, path):
    ...
    with open(path, "w", newline="", encoding="utf-8-sig") as f:
        ...
        sd = params.get("sensor_dist", 60.0)
        w.writerow(["传感器距井口 %g m: S1正北 S2正西 S3正南 S4正东; "
                    "告警阈值 10 ppm" % (sd,)])
...
def main():
    ...
    fig.suptitle(f"油井 H2S 泄漏核心可视化: 传感器 + 重气云团 + 应急分区 "
                 ...
                 f"{base._wind_label(params)}, 传感器距井口 {params.get('sensor_dist', 60.0):.0f} m)",
