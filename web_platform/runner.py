...
# 传感器布局(最新要求: 正北/正西/正南/正东, 距井口 60 m, 构成边长 60 m 的正方形)
SENSOR_LAYOUT = [
    dict(name="S1", E=0.0,   N=60.0,  z=1.5, desc="正北"),
    dict(name="S2", E=-60.0, N=0.0,   z=1.5, desc="正西"),
    dict(name="S3", E=0.0,   N=-60.0, z=1.5, desc="正南"),
    dict(name="S4", E=60.0,  N=0.0,   z=1.5, desc="正东"),
]
...
def make_params(form):
    ...
    p.update(
        ...
        sensors=[dict(s) for s in SENSOR_LAYOUT],
...
def run_job(job):
    ...
    try:
        ...
        rv.SENSOR_LAYOUT[:] = [dict(s) for s in SENSOR_LAYOUT]
        ...
        if nt >= 4:
            ...
            fig.suptitle("油井 H2S 泄漏核心可视化: 传感器 + 重气云团 + 应急分区 "
                         "(源强 %g kg/s, 风速 %g m/s, %s, 传感器距井口 60 m)"
...
def build_results(job, field, p, slices, stats):
    ...
    summary = dict(
        ...
        sensor_layout="S1正北 S2正西 S3正南 S4正东, 距井口 60 m",
    )
